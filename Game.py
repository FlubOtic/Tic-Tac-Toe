...
def turn(pos, xo, block, board, screen):
    for i in range(3):
        for j in range(3):
...
def GameLoop():
    ...
    for i in range(3):
        ...
        for j in range(3):
            ...
            # pygame.draw.circle(screen, (0, 0, 0), block[j][i], 50)
            x += SCREEN_WIDTH/6
        y += SCREEN_HEIGHT/6
    ...
    while not crash:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                turned = turn(pygame.mouse.get_pos(), player_turn, block, board, screen)
                won = board.won

                if player_turn == player1 and turned and not won:
                    player_turn = player2
                elif turned and not won:
                    player_turn = player1
        ...
        if won:
            screen.fill((255, 255, 255))
            draw_text(screen, player_turn + " Won!", 50, (SCREEN_WIDTH/2, SCREEN_HEIGHT/2 - 50))
            draw_text(screen, "Press Enter To Play Again", 40, (SCREEN_WIDTH/2, SCREEN_HEIGHT/2))
